...
function genSelfMessageId(user_id, seq, random) {
    const buf = Buffer.alloc(8);
    buf.writeUInt32BE(user_id), buf.writeUInt16BE(seq, 4), buf.writeUInt16BE(random, 6);
...
function log(any) {
    console.log(util.inspect(any, {depth: 20, showHidden: true, maxArrayLength: 1000, maxStringLength: 5000}));
}

module.exports = {
    rand, uuid, now, md5, timestamp, buildApiRet, toInt, checkUin,
