 ...
 */
async function _downloadFromWeb(url, headers, timeout, proxy, mime_type, maxsize = MAX_UPLOAD_SIZE, redirect = false) {
    if (timeout > 0 === false)
        timeout = 60;
...
async function downloadWebImage(url, proxy, timeout, headers = null) {
    return await _downloadFromWeb(url, headers, timeout, proxy, "image");
}
async function downloadWebRecord(url, proxy, timeout, headers = null) {
    return await _downloadFromWeb(url, headers, timeout, proxy, "", 0xfffffff);
}
