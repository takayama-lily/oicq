...
const {buildMessage, parseMessage, genCQMsg} = require("./message");
const {uploadImages, uploadPtt, uploadMultiMessage, downloadMultiMsg} = require("./service");
...
async function sendMsg(target, message, escape, type) {
    ...
    const _sendMsg = async(elems, long = false)=>{
        if (long)
            elems = await toLongMessageElems.call(this, type?common.code2uin(target):target, elems);
    ...
    for (let [elem, o] of map) {
        ...
        switch (o.type) {
            case "stat":
                ...
                break;
            case "ptt":
                const ptt = await uploadPtt.call(this, target, elem);
                elem.groupFileKey = ptt.fileKey;
...
async function getMsgFromElems(rich) {
    ...
    if (typeof res === "string") {
        res = await downloadMultiMsg.call(this, res, 1);
