    ...
    for (let v of elems) {
        ...
        switch (type) {
            ...
            case "ptt":
                ...
                raw_message += toCQRecord(msg.data.file);
                break;
        }
    }
...
async function audioTrans(cache_filepath, file) {
    ...
    return new Promise((resolve, reject)=>{
        spawn.exec(`ffmpeg -y -i ${filepath} -ac 1 -ar 8000 ${cache_filepath}`, async(error, stdout, stderr)=>{
            if (typeof file !== "string")
                fs.unlink(filepath, ()=>{});
            if (error || stderr) {
                ...
                resolve(await fs.promises.readFile(cache_filepath));
            }
        })
    })
}
...
async function buildRecordMessage(chain, data, is_group) {
    ...
    if (!file) return false;
    file = file.trim();
    let buf, md5, size, ext;
    try {
        file = file.replace(/^file:\/{2,3}/, "");
        const cache_filename = Buffer.from(file, "utf-8").toString('base64');
        ...
        if (head.includes("SILK"))
            ext = ".slk";
        else if (head.includes("AMR"))
            ext = ".amr";
    } catch (e) {
        process.OICQ.logger.warn(`音频文件 ${file} 处理失败。`);
        return false;
    }
    md5 = common.md5(buf), size = buf.length;
    chain[0].unshift({
...
async function buildMessage(message, escape, is_group) {
    const chain = [[]], tasks = [];
    const stat = {
        length: 0, at_cnt: 0, face_cnt: 0, img_cnt: 0, has_ptt: false
    };
