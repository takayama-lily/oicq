    ...
    for (let v of elems) {
        ...
        switch (type) {
            ...
            case "text":
                if (o.attr7Buf) {
                    msg.type = "bface";
                } else if (o.attr6Buf && o.attr6Buf[1] === 1) {
                    msg.type = "at";
                    if (o.attr6Buf[6] === 1)
                        msg.data.qq = "all"
                    else
            ...
            case "commonElem":
                if (o.serviceType === 3) {
                    const flash = pb.decode("HummerCommelem", o.pbElem);
                    msg.type = "flash";
        ...
        if (msg.type) {
            ...
            if (msg.type === "text")
                ...
                raw_message += `[CQ:${msg.type},${querystring.stringify(msg.data, ",", "=", {encodeURIComponent: (s)=>s.replace(/&|,/g, s_s)})}]`;
        }
    }
...
function parseImage(o) {
    let data = {};
    if (o.picMd5) {
        data.file = o.picMd5.toString("hex") + o.fileLen;
        if (o.origUrl)
