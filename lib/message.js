...
function buildLocationMessage(chain, data) {
    ...
    // obj = Buffer.concat([BUF1, zlib.deflateSync(JSON.stringify(obj))]);
    // const elem = {lightApp: {data: obj}};
    // chain.set(elem, {
    //     type: "json", text: "收到[[应用]地图]消息，请升级QQ版本查看"
    // });
    // return true;
}
...
async function buildMessageFromString(chain, message, escape, group_id, stat) {
    ...
    for (let v of res) {
        ...
        for (let k of Object.keys(cq))
            cq[k] = cq[k].replace(/&#44;|&#91;|&#93;|&amp;/g, unescapeCQInside);

        elem = await buildElement.call(this, chain, cq.cqtype.trim(), cq, group_id, stat)
...
//----------------------------------------------------------------------------------------------------
module.exports = {
    parseMessage, buildMessage, genCQMsg
};
