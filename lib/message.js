...
async function buildRecordMessage(chain, data, is_group) {
    ...
    try {
        if (file.startsWith("http://") || file.startsWith("https://")) {
            const filename = Buffer.from(file, "utf-8").toString('base64');
            const filepath = path.join(process.OICQ.config.cache_root, "record", filename);
            if (data.cache !== "0") {
                try {
            ...
            if (!buf) {
                buf = await downloadWebRecord(file);
        ...
        if (head.includes("SILK"))
            ...
            process.OICQ.logger.warn(`语音文件 ${file} 不是slk或amr格式。`);
            return false;
        }
    } catch (e) {
        console.log(e)
        process.OICQ.logger.warn(`获取语音文件 ${file} 失败。`);
        return false;
    }
    md5 = common.md5(buf), size = buf.length;
    chain[0].unshift({
