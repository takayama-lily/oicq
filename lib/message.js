...
async function buildImageMessage(chain, file, url, is_group) {
    ...
    file = file.trim();
    let buf, md5, size, from_web = false;
    if (file.startsWith("http://") || file.startsWith("https://")) {
        md5 = crypto.randomBytes(16), size = common.rand(6);
        from_web = true;
    } else {
    ...
    for (let v of res) {
        ...
        switch (cq.cqtype.trim()) {
            ...
            case "face":
                ...
                break;
            case "image":
                const task = await buildImageMessage(chain, cq.file, cq.url, is_group);
                if (task)
                    ++img_cnt;
        ...
        for (let v of message) {
            ...
            switch (v.type) {
                ...
                case "face":
                    ...
                    break;
                case "image":
                    const task = await buildImageMessage(chain, v.data.file, is_group)
                    if (task)
                        ++img_cnt;
                    if (task instanceof Promise)
                        tasks.push(task);
