...
const {handlePrivateMsg} = require("./message/recv");
        ...
        for (let v of rsp[5]) {
            ...
            for (let msg of v[4]) {
                ...
                //私聊消息
                    ...
                    (async()=>{
                        try {
                            const m = await handlePrivateMsg.call(this, type, head, msg[2], msg[3]);
                            if (m && m.raw_message) {
                                this.logger.info(`recv from: [Private: ${m.user_id}(${m.sub_type})] ` + m.raw_message);
                                this.em("message.private." + m.sub_type, m);
                            }
                        } catch (e) {
                            this.logger.debug(e);
                        }
