        ...
        for (let msg of v.messages) {
            ...
            if (type === 33) {
                (async()=>{
                    ...
                    event.emit(this, "notice.group.increase", {
                        group_id, user_id,
            ...
            if ([141, 166, 167].includes(type))
                chat.onPrivateMsg.call(this, user_id, head, body, update_flag);
        }
    }

    if (rubbish.length) {
        this.nextSeq();
        this.writeUNI("MessageSvc.PbDeleteMsg", pb.encode("DeleteMessageRequest", {rubbish}));
    }
    if (o.syncFlag !== 2)
...
async function getNewFriend() {
    ...
    try {
        ...
        event.emit(this, "request.friend.add", {
            user_id,
...
async function getNewGroup() {
    ...
    try {
        ...
        if (v.msg.groupMsgType === 1) {
            ...
            event.emit(this, "request.group.add", {
                group_id, user_id,
            ...
            event.emit(this, "request.group.invite", {
                group_id, user_id,
                group_name: v.msg.groupName,
...
async function friendAction(flag, approve = true, block = false) {
...
async function getGMI(group_id, user_id, c) {
    c.nextSeq();
...
async function getSI(user_id, c) {
    const arr = [
        null,
...
module.exports = {
    parseIncomingPacket, friendAction, groupAction,
