...
function onPushNotify(blob) {
    ...
    switch (parent[5]) {
        ...
        case 525:
            ...
            return sysmsg.getNewGroup.call(this, notify_types[parent[5]]);
        case 187:
        ...
        for (let v of rsp[5]) {
            ...
            for (let msg of v[4]) {
                ...
                if (!this.sync_finished)
                    continue;
                let user_id = head[1];
                if (user_id === this.uin && (this.ignore_self || user_id !== head[2]))
                    continue;
                ...
                if (type === 33) {
                    (async()=>{
                        ...
                        user_id = head[15];
                        const id = BigInt(group_id) * BigInt(user_id) * BigInt(head[5]);
                        if (this.notify33cache.has(id) || common.timestamp() - head[6] >= 10)
                            return;
                        this.notify33cache.add(id);
                        setTimeout(()=>{
                            this.notify33cache.delete(id);
                        }, 10000);
...
module.exports = {
    parseIncomingPacket, getMsg, calcMsgCnt
};
