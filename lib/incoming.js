...
async function decodeGroupMessageEvent(blob, c) {
    ...
    try {
        await c.hasGroup(group_id);
        if (!c.group_member_list.has(group_id) || !c.group_member_list.get(group_id).has(user_id))
            await c.getGroupMemberList(group_id, true);
        ...
        if (v[2] === 528) {
            ...
            if (v[6][4] === 0x14) {
                ...
                if (data.optGeneralGrayTip) {
                    let user_id, receiver_id, action, suffix;
                    ...
                    event.emit(c, "notice.group.poke", {
                        group_id, user_id, receiver_id, action, suffix
                    });
                }
            }
            // common.log(v[6].toString("hex").replace(/(.)(.)/g, '$1$2 '))
