        ...
        for (let msg of v.messages) {
            ...
            if (body.richText && body.richText.elems && body.richText.attr) {
                ...
                if (body.richText.ptt)
                    body.richText.elems.push({ptt: body.richText.ptt});
                let res;
                (async()=>{
                    try {
                        res = await getMsgFromElems(body.richText.elems, c);
                    } catch (e) {return}
    ...
    try {
        if (chain[0].type === "file") {
            ...
            v.data = {
                ...
                size:   parseInt((BigInt(v.data.fileSize.high)<<32n)|BigInt(v.data.fileSize.low)),
