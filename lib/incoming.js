        ...
        for (let msg of v.messages) {
                ...
                (async()=>{
                    ...
                    try {
                        ...
                        if (user_id === c.uin) {
                            ...
                            // c._getGroupMemberList(group_id);
                        } else {
                            ginfo.member_count++;
            ...
            if (type === 141) {
                ...
                if (head.c2cTmpMsgHead && head.c2cTmpMsgHead.groupCode) {
                    ...
                    Object.assign(sender, (await c.getGroupMemberInfo(group_id, user_id)).data);
                }
            ...
            if (!c.fl.has(user_id)) {
                const stranger = (await c.getStrangerInfo(user_id)).data;
                if (stranger) {
...
async function decodeGroupMessageEvent(blob, c) {
    ...
    try {
        const ginfo = (await c.getGroupInfo(group_id)).data;
        if (!c.gml.has(group_id))
            c._getGroupMemberList(group_id);
        if (user_id !== 80000000) {
            const minfo = (await c.getGroupMemberInfo(group_id, user_id)).data;
            if (common.timestamp() - minfo.update_time > 3600)
                c.getGroupMemberInfo(group_id, user_id, true);
        }
        if (common.timestamp() - ginfo.update_time > 3600)
    ...
    if (user_id === 80000000) {
        ...
        try {
            user = c.gml.get(group_id).get(user_id);
            user.card = card;
...
function decodeGroupMemberListResponse(blob, c) {
    ...
    if (!parent[3].length)
        return 0;
    const member_list = c.gml.get(group_id);
...
function decodeGroupMemberInfoResponse(blob, c) {
    ...
    if (!o.role) return;
    if (!c.gml.has(group_id))
        c.gml.set(group_id, new Map());
    const uin = toInt(o.uin);
    if (o.sex === undefined) o.sex = -1;
    c.gml.get(group_id).set(uin, {
        group_id:           group_id,
        user_id:            uin,
        nickname:           o.nickname,
        card:               o.card,
...
function decodeNewFriendResponse(blob, c) {
    ...
    for (let v of list) {
        ...
        c.logger.info(`收到 ${user_id}(${v.msg.reqUinNick}) 的加好友请求`);
    ...
    for (let v of list) {
        if (v.msg.subType === 1) {
            ...
            if (v.msg.groupMsgType === 1) {
                const user_id = toInt(v.reqUin);
                c.logger.info(`用户 ${user_id}(${v.msg.reqUinNick}) 请求加入群 ${group_id}(${v.msg.groupName})`);
                ...
                c.logger.info(`用户 ${user_id}(${v.msg.actionUinNick}) 邀请你加入群 ${group_id}(${v.msg.groupName})`);
        ...
        if (v[2] === 528) {
            ...
            if (data[0] === 0x8A || data[0] === 0x8B) {
                ...
                (async()=>{
                    ...
                    const user = (await c.getStrangerInfo(user_id, c)).data;
                    if (user) {
                        user.remark = nickname;
                        c.fl.set(user_id, user);
                    }
                    c.strangers.delete(user_id);
                ...
                (async()=>{
                    if (!(await c.getGroupInfo(group_id, true)).data) {
                        for (let [k, v] of c.strangers) {
                            if (v.group_id === group_id)
                                c.strangers.delete(k);
                        }
                    }
                })();
            }
...
function decodeOnlinePushTransEvent(blob, c, seq) {
    ...
    if (o.msgType === 44) {
        if (buf[5] === 0 || buf[5] === 1) {
            ...
            const set = buf[10] > 0;
            try {
                c.gml.get(group_id).get(user_id).role = (set ? "admin" : "member");
            } catch (e) {}
            ...
            try {
                ...
                c.gml.get(group_id).get(operator_id).role = "member";
                c.gml.get(group_id).get(user_id).role = "owner";
            } catch (e) {}
    ...
    if (o.msgType === 34) {
        ...
        if (c.strangers.has(user_id) && c.strangers.get(user_id).group_id === group_id)
            c.strangers.delete(user_id);
