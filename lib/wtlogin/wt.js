...
async function tokenLogin() {
    try {
        this.sig.srm_token = await fs.promises.readFile(path.join(this.dir, "token"));
        // this.sig.ticket_key = buf.slice(0, 16);
        // this.sig.srm_token = buf.slice(16);
    } catch {}
    ...
    const body = new Writer()
        ...
        .writeBytes(t(0x516))
        .read();
    const pkt = build0x0APacket.call(this, "wtlogin.exchange_emp", buildOICQPacket.call(this, body, false), 2);
    try {
        var blob = await this.send(pkt);
    } catch {}
    decodeLoginResponse.call(this, blob);
...
function decodeT119(data) {
    ...
    //fs.writeFile(path.join(this.dir, "token"), this.sig.srm_token, ()=>{});
}
