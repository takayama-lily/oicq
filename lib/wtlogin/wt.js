...
async function deviceLogin(t402) {
    ...
    const body = new Writer()
        ...
        .writeBytes(tlv[0x401](common.md5(Buffer.concat([
            this.device.guid, Buffer.from("stMNokHgxZUGhsYp"), t402
        ]))))
        .read();
    const sso = commonSSO.call(this, "wtlogin.login", commonOICQ.call(this, body));
    try {
...
async function register() {
    ...
    const SvcReqRegister = jce.encodeStruct([
        this.uin,
        7, 0, "", 11, 0, 0, 0, 0, 0, 0,
        this.device.version.sdk, 1, "", 0, BUF0, this.device.guid, 2052, 0, this.device.model, this.device.model,
        this.device.version.release, 1, 1551, 0, null, 0, 31806887127679168n, "", 0, "MIUI",
...
function decodeT119(data) {
    const reader = Readable.from(tea.decrypt(data, this.device.tgtgt_key), {objectMode:false});
    reader.read(2);
