...
const { downloadFromWeb, highwayUploadStream } = require("./service");
...
async function _getImgBuf(file) {
    ...
    if (file instanceof Buffer) {
        ...
        if (file.startsWith("base64://")) {
            ...
            buf = await downloadFromWeb(file);
        } else {
...
async function setPortrait(file) {
    ...
    await highwayUploadStream.call(this, Readable.from(buf, { objectMode: false }), {
        cmd: 5,
        md5: md5(buf),
        size: buf.length,
