...
const push528 = {
    0x8A: function(buf, time) {
        ...
        this.em("notice.friend.recall", {
            user_id, message_id: chat.genGroupMessageId(user_id, data.msgSeq, data.msgRandom), time
        });
    },
...
const push732 = {
    0x0C: function(group_id, buf, time) {
        ...
        try {
            if (user_id === 0)
                this.gl.get(group_id).shutup_time_whole = duration?0xffffffff:0;
    ...
    0x11: function(group_id, buf, time) {
        ...
        const message_id = chat.genGroupMessageId(group_id, msg.seq, msg.msgRandom);
