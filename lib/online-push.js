...
const {handleGroupMsg, handleDiscussMsg} = require("./message/recv");
...
async function onGroupMsg(blob, seq) {
    ...
    try {
        const msg = await handleGroupMsg.call(this, o[1][1], o[1][3]);
...
async function onDiscussMsg(blob, seq) {
    ...
    if (!this.sync_finished)
        return;
    try {
        const msg = await handleDiscussMsg.call(this, o[1][1], o[1][3]);
        if (msg && msg.raw_message) {
            const sender = msg.sender;
            this.logger.info(`recv from: [Discuss: ${msg.discuss_name}(${msg.discuss_id}), Member: ${sender.card}(${msg.user_id})] ` + msg.raw_message);
