...
function buildPrivateMessageRequestPacket(user_id, routing, message, escape, c) {
    c.nextSeq();
    ...
    const body = pb.encode("SendMessageRequest", {
        ...
        msgBody: {
            richText: {elems: common.buildMessage(message, escape, false)}
        },
        msgSeq:     c.seq_id,
        msgRand:    random,
...
function buildSendFriendMessageRequestPacket(user_id, message, escape, c) {
    const routing = {  
        c2c: {toUin: user_id}
    };
    return buildPrivateMessageRequestPacket(user_id, routing, message, escape, c);
}
function buildSendTempMessageRequestPacket(group_id, user_id, message, escape, c) {
    const routing = {
        grpTmp: {
            groupUin: group_id,
            toUin:    user_id,
...
function buildSendGroupMessageRequestPacket(group_id, message, escape, c) {
    c.nextSeq();
    const random = common.rand();
    c.curr_msg_id = common.genMessageId(group_id, c.seq_id, random);
    const body = pb.encode("SendMessageRequest", {
        routingHead: {
            grp: {groupCode: group_id}
        },
        contentHead: {pkgNum: 1},
        msgBody: {
            richText: {elems: common.buildMessage(message, escape, true)}
        },
        msgSeq:     c.seq_id,
        msgRand:    random,
        SyncCookie: BUF0,
