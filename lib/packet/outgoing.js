...
const {uploadImages, uploadGroupMessage} = require("./service");
...
const CMD = {
    ...
    MULTI_APPLY_UP: "MultiMsg.ApplyUp", //send,recv
}
...
async function buildGroupLongMessageRequestPacket(group_id, chunks, c) {
    const msg = [];
    for (let v of chunks) {
        msg.push({
            head: {
    ...
    const buf = zlib.gzipSync(pb.encode("PbMultiMsgTransmit", {
        msg, pbItemList: [{
            fileName: "MultiMsg",
    ...
    let resp = await c.send(buildMultiApplyUpRequestPacket(group_id, md5, buf.length, 1, c));
    resp = resp.MultimsgApplyupRsp[0];
    await uploadGroupMessage(c.uin, resp.uint32UpIp, resp.uint32UpPort, {
        buf, md5, key: resp[0].msgSig
    });

    const templete = `<?xml version='1.0' encoding='UTF-8' standalone='yes' ?>
<msg serviceID="35" templateID="1" action="viewMultiMsg"
        brief="这是一段长消息这是一段长消息这是一段长消息这是一段长消息..."
        m_resid="${resp.msgResid}"
        m_fileName="${common.timestamp()}" sourceMsgId="0" url=""
        flag="3" adverSign="0" multiMsgFlag="1">
    <item layout="1">
        <title>这是一段长消息这是一段长消息这是一段长消息这是一段长消息...</title>
...
function buildMultiApplyUpRequestPacket(group_id, md5, size, bu, c) {
    c.nextSeq();
    const body = pb.encode("MultiReqBody", {
        ...
        buType:         bu,
        multimsgApplyupReq: [{
            dstUin:     common.code2uin(group_id),
            msgSize:    size,
            msgMd5:     md5,
            msgType:    3,
        }],
    });
    return commonUNI(c, CMD.MULTI_APPLY_UP, body);
}
...
module.exports = {
    ...
    // group operation
    buildEditSpecialTitleRequestPacket, buildGroupOperationRequestPacket, buildSetGroupAdminRequestPacket,
