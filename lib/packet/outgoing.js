...
/**
 * @param {Client} c 
 * @param {Number} command_id UInt16
 * @param {Buffer} body 
 * @returns {Buffer}
 */
function commonOICQ(c, command_id, body) {
    // ecdh.gen();
    body = new Writer()
        .writeU8(0x01)
        .writeU8(0x01)
 ...
 * @returns {Buffer}
 */
function commonSSO(c, command_name, ext_data, body) {
 ...
 * @returns {Buffer}
 */
function commonLogin(uin, body_type, key, ext_data, body) {
    body = new Writer()
 ...
 * @param {Client} c 
 * @param {String} command_name 
 * @param {Number} body_type UInt8 1?
 * @param {Buffer} ext_data 
 * @param {Buffer} body 
 * @returns {Buffer}
 */
function commonUNI(c, command_name, body_type, ext_data, body) {
    c.logger.trace(`send:${command_name} seq:${c.seq_id}`);
    let uni = new Writer()
...
function buildPasswordLoginRequestPacket(c) {
    ...
    const device_info_data = pb.encode("DeviceInfo", {
    ...
    const req = commonOICQ(
        c, 0x0810, stream.read()
    );
    const sso = commonSSO(
        c, "wtlogin.login", BUF0, req
    );
    return commonLogin(
        c.uin, 2, Buffer.alloc(16), BUF0, sso
    );
}
...
function buildHeartbeatRequestPacket(c) {
    c.nextSeq();
    const sso = commonSSO(
        c, "Heartbeat.Alive", BUF0, BUF0
    );
    return commonLogin(
        c.uin, 0, BUF0, BUF0, sso
    );
}
...
function buildClientRegisterRequestPacket(c) {
    ...
    const sso = commonSSO(
        c, "StatSvc.register", c.sign_info.tgt, body
    );
    return commonLogin(
        c.uin, 1, c.sign_info.d2key, c.sign_info.d2, sso
    );
}
...
function buildConfPushResponsePacket(type, seq, jcebuf, c) {
    ...
    return commonUNI(
        c, "ConfigPushSvc.PushResp", 1, BUF0, body
    );
}
...
function buildGetMessageRequestPacket(sync_flag, c) {
    ...
    return commonUNI(
        c, "MessageSvc.PbGetMsg", 1, BUF0, body
    );
}
...
function buildFriendListRequestPacket(start, limit, c) {
    ...
    return commonUNI(
        c, "friendlist.getFriendGroupList", 1, BUF0, body
    );
}
...
function buildGroupListRequestPacket(c) {
    ...
    return commonUNI(
        c, "friendlist.GetTroopListReqV2", 1, BUF0, body
    );
}
...
function buildGroupMemberListRequestPacket(group_uin, group_code, next_uin, c) {
    ...
    return commonUNI(
        c, "friendlist.GetTroopMemberListReq", 1, BUF0, body
    );
}
...
function buildSendFriendMessageRequestPacket() {}
function buildSendGroupMessageRequestPacket() {}

function buildGroupRequestRequestPacket() {}
function buildFriendRequestRequestPacket() {}
