...
async function decodeGroupMessageEvent(blob, c) {
    ...
    else if (Date.now() - c.group_list.get(group_id).member_list_uptime > 900)
        c.getGroupMemberList(group_id, true);
    ...
    if (user_id === 80000000) {
        ...
        try {
            c.group_member_list.get(group_id).get(user_id).card = card;
...
const decoders = new Map([
    ...
    [CMD.MULTI_APPLY_UP,    decodeMultiApplyUpRequestResponse],
]);
