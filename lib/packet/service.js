 ...
 * @returns {Buffer[]}
 */
function buildImageUploadRequestPackets(uin, buf, md5, key, cmd, seq = common.rand()) {
    uin = uin.toString();
...
async function highwayUpload(uin, ip, port, data, cmd) {
    ...
    return new Promise((resolve)=>{
        ...
        var packets = buildImageUploadRequestPackets(uin, data.buf, data.md5, data.key, cmd);
    })
}
...
async function uploadGroupLongMessage() {
