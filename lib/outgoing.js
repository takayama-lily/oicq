    ...
    if (images.length > 0) {
        try {
            ...
            for (let i = 0; i < resp.msgTryUpImgRsp.length; ++i) {
                ...
                if (images[i].type === "flash") {
                    flash = images[i];
                    flash.resId = v.upResid;
                } else {
        ...
        elems = [
            {
                commonElem: {
                    serviceType: 3,
                    pbElem: pb.encode("MsgElemInfoServtype3", {
                        flashC2cPic: {
                            fileLen:    flash.size,
                            filePath:   flash.resId,
                            resId:      flash.resId,
                            picMd5:     flash.md5,
                            oldPicMd5:  false
                        }
...
<msg serviceID="35" templateID="1" action="viewMultiMsg"
    ...
    return [
        {
            richMsg: {
                template1: Buffer.concat([BUF1, zlib.gzipSync(templete)]),
                serviceId: 35,
                msgResId:  BUF0,
            }
        },
        {
            text: {str: "你的QQ暂不支持查看[转发多条消息]，请期待后续版本。"}
        },
        {
            generalFlags: {
                longTextFlag:  1,
