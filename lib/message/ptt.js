 ...
 * @param {number} target
 * @param {import("../../client").ImgPttElem["data"]} cq 
...
async function genPttElem(target, cq) {
    ...
    if (!["0", "false", "no"].includes(String(cache))) {
        try {
            ...
            return await uploadPtt.call(this, target, buf);
        } catch { }
    }
    ...
    if (file instanceof Buffer) {
        ...
        const head = await read7Bytes(tmp_file);
        ...
        // 音频转换(生成缓存文件)->发送
        buf = await audioTrans.call(this, cache_file, tmp_file);
        return await uploadPtt.call(this, target, buf);
    }
 ...
 * @returns {Promise<Buffer>}
 */
async function audioTrans(cache_file, tmp_file) {
 ...
 */
async function read7Bytes(filepath) {
 ...
 * @returns {Promise<Buffer>}
 */
async function uploadPtt(target, buf) {
