...
class ImageBuilder {
    ...
    async download() {
        ...
        try {
            ...
            this.c.logger.warn(`下载网络图片失败 ${this.address} 失败。`);
        ...
        try {
            ...
            this.buf = null;
            return this.c.logger.warn(`${this.address} 不是有效的图片。`);
        }
    ...
    async buildNested(obj) {
        ...
        // 网络图片
            ...
            this.address = file, this.proxy = proxy == "1", this.timeout = timeout, this.headers = headers;
            try {
                if (cache == "0")
...
async function imageStore(group_id, imgs) {
    ...
    for (let i = 0; i < imgs.length; ++i) {
        ...
        imgs[i].key = v[8].raw;
        tasks.push(highwayUpload.call(this, v[6], v[7], imgs[i], 2));
    }
...
async function offPicUp(user_id, imgs) {
    ...
    for (let i = 0; i < imgs.length; ++i) {
        ...
        imgs[i].key = v[9].raw;
        tasks.push(highwayUpload.call(this, v[7], v[8], imgs[i], 1));
    }
    await Promise.all(tasks);
}
...
async function uploadImages(target, imgs, c2c) {
    ...
    while (imgs.length > n) {
        try {
            await (c2c ? offPicUp : imageStore).call(this, target, imgs.slice(n, n + 20));
        } catch { }
        n += 20;
    }
}
