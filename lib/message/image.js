...
class ImageBuilder {
    ...
    async download() {
        ...
        try {
            ...
            const res = await downloadFromWeb(this.address, this.headers);
            const id = setTimeout(()=>{
                this.c.logger.warn("download timeout");
                res.destroy();
            }, this.timeout * 1000);
            ...
            this.deleteTmpFile();
            this.c.logger.warn("图片下载失败：" + this.address);
    ...
    async buildNested(cq) {
        ...
        // 网络图片
            ...
            this.address = file;
            this.timeout = parseInt(timeout) || 60;
