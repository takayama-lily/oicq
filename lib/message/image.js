 ...
 * 图片处理流程
 * 
 *           |-> localfile  -> get stat(md5,size,hw) -> createReadStream ->|
 *           |-> httpfile   -> tmplocalfile&get stat -> createReadStream ->|
 * construct |->                                                         ->| uploadImages(20并发) -> not exists? -> highwayUploadStream(20并发)
 *           |-> base64file -> Buffer -> get stat    -> Readable.from    ->|
 *           |-> Buffer     -> get stat              -> Readable.from    ->|
 */
class ImageBuilder {
    ...
    async download() {
        ...
        try {
            ...
            this.buf = await downloadFromWeb(this.address, this.timeout, this.headers);
        ...
        this.setNested();
        if (this.filepath) {
            const file = buildImageFileParam(this.md5.toString("hex"), this.size, this.width, this.height, this.type);
    ...
    async buildNested(cq) {
        let { file, url, cache, timeout, headers } = cq;
        if (!file)
        ...
        // base64图片
            ...
            try {
                ...
                try {
                    ...
                    this.buf = await fs.promises.readFile(file);
                } catch (e) {
                    this.c.logger.warn(`获取本地图片失败 ${file}`);
    ...
    while (imgs.length > n) {
        try {
            ...
            for (let i = n; i < imgs.length; ++i) {
                ...
                tasks.push(highwayUploadStream.call(this, Readable.from(imgs[i].buf, { objectMode: false }), imgs[i], ip, port));
            }
