...
const { parseC2CMessageId, parseGroupMessageId, genMessageUuid, genC2CMessageId } = common;
...
class Builder {
     ...
     */
    buildPbSendMsgPkt(rich, content = PB_CONTENT) {
        this.seq = this.c.seq_id + 1;
        this.random = randomBytes(4).readUInt32BE();
        if (this.anon)
    ...
    async send(rich, flag = false) {
        ...
        if (retcode === 0) {
            if (this.type === 0) { //私聊
                message_id = genC2CMessageId(this.target, this.seq, this.random, rsp[3]);
            }
    ...
    async sendByFrag() {
        ...
        for (let fragment of fragments) {
            ...
            const body = this.buildPbSendMsgPkt({ 2: fragment }, content);
