...
class Builder {
    ...
    async buildImageElem(cq) {
        ...
        if (file.startsWith("http://") || file.startsWith("https://")) {
            ...
            try {
                ...
                const task = (async()=>{
                    const buf = await downloadWebImage.call(this.c, file, proxy=="1", timeout);
                    if (buf) {
                        img.buf = buf;
                        img.size = buf.length;
    ...
    async buildPttElem(cq) {
        ...
        try {
            ...
            if (!buf) {
                if (file.startsWith("http://") || file.startsWith("https://"))
                    file = await downloadWebRecord.call(this.c, file, proxy=="1", timeout);
                else if (file.startsWith("base64://"))
    ...
    async buildMusicElem(cq) {
        const {type, id} = cq;
        try {
            const buf = await music.build.call(this.c, this.target, type, id, this.type);
            this.b77.push(buf);
        } catch (e) {
            // console.log(e)
    ...
    async exec(message, escape) {
        if (typeof message[Symbol.iterator] === "function" && typeof message !== "string") {
            ...
            await this.buildFromString(message, escape);
        }
