...
const {uploadPtt, uploadImages, setPrivateImageNested, setGroupImageNested} = require("./img-ptt-up-down");
const pb = require("../pb");
...
class Builder {
    ...
    async buildImageElem(cq) {
        let {file, url, cache, type, timeout, proxy, headers} = cq;
        if (!file) return;
        ...
        // 网络图片
            ...
            try {
                ...
                const task = (async()=>{
                    try {
                        var buf = await downloadWebImage(file, proxy=="1", timeout, headers);
                    } catch (e) {
        ...
        try {
            ...
            if (!buf) {
                if (file.startsWith("http://") || file.startsWith("https://"))
                    file = await downloadWebRecord(file, proxy=="1", timeout, headers);
                else if (file.startsWith("base64://"))
    ...
    async buildReplyElem(cq) {
        ...
        try {
            ...
            this.c.logger.warn("incorrect reply id");
            return;
        }
        try {
    ...
    buildPokeElem(cq) {
        ...
        if (type === 126) {
            nested[4] = id;
            nested[5] = Buffer.from([230, 149, 178, 233, 151, 168]);
            nested[6] = Buffer.from([55, 46, 50, 46, 48]);
        }
        this.elems.push({
            53: {
                1: 2,
    ...
    async buildElem(type, data) {
        ...
        switch (type) {
            ...
            case "json":
                if (data.data)
                    this.buildJsonElem(data.data);
                break;
            case "xml":
                if (data.data)
                    this.buildXmlElem(data.data, 60);
                break;
            case "node":
