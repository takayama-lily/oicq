...
const {uploadPtt, uploadImages, setPrivateImageNested, setGroupImageNested} = require("./storage");
...
async function buildMessage(target, message, escape, is_group) {
    ...
    const stat = {
        ...
        is_forward: false, type: "stat", tasks: [], imgs: []
    };
    chain.set("stat", stat);
    ...
    await completeImages.call(this, target, stat.imgs, is_group);
    return chain;
}
