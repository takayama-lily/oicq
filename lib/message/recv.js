...
const pb = require("../pb");
const { genC2CMessageId, genGroupMessageId, timestamp, parseFunString, code2uin, log } = require("../common");
...
class Parser {
    ...
    /**
     * @type {import("../../client").MessageElem[]}
     */
    message = [];
    raw_message = "";
    brief = "";
     ...
     * @param {import("../ref").Proto} ptt 
     */
    async parseMsg(elems, ptt) {
    ...
    async parseExclusiveElem(type, elem) {
        ...
        case 5: //transElem
            msg.type = "file";
            let v = pb.decode(elem[2].raw.slice(3))[7];
            v = v[2];
            let rsp = await getGroupFileUrl.call(this, this.gid, v[1], v[2].raw);
            msg.data = {
                name: String(v[4].raw),
                ...
                duration: v[5],
                busid: this.gid.toString(36) + "-" + v[1],
    ...
    parsePartialElem(type, elem) {
        ...
        switch (type) {
        case 1: //text&at
            msg.data.text = String(elem[1].raw);
            if (elem[3] && elem[3].raw[1] === 1) {
                ...
                brief = "@" + msg.data.text ? msg.data.text : msg.data.qq;
            } else {
                if (!msg.data.text)
                    return;
                msg.type = "text";
        ...
        case 34: //sface
            msg.type = "sface";
            msg.data.id = elem[1];
            msg.data.text = this.getNextText();
            brief = msg.data.text;
            break;
        default:
            return;
        }
        if (msg.type === "text" && this.message.length > 0 && this.message[this.message.length - 1].type === "text") { //合并文本节点
    ...
    async parseElems(elems) {
        ...
        while (true) {
            ...
            if (type === 16) { //extraInfo 额外情报
                ...
                if (elem[6] === 1 && elem[7]) {
                    const buf = await downloadMultiMsg.call(this, elem[7].raw, 1);
                ...
                case 19: //video
                case 51: //json
                    await this.parseExclusiveElem(elem);
                    break;
                case 53: //commonElem
                    if (elem[1] === 3) { //flash
    ...
    async parseReplyElem(elem) {
        ...
        try {
            ...
            if (this.gid) {
                let m = await getGroupMsgs.call(this, this.gid, elem[1], elem[1]);
                m = m[0];
                msg.data.id = genGroupMessageId(this.gid, elem[2], elem[1], m[3][1][1][3], m[1][6]);
            } else {
                let random = elem[8][3];
                if (typeof random === "bigint")
                    random = parseInt(random & 0xffffffffn);
                msg.data.id = genC2CMessageId(this.uid, elem[1], random, elem[3]);
            }
            this.message.unshift(msg);
            this.raw_message = genCQMsg(msg) + this.raw_message;
            this.brief = "[回复]" + this.brief;
        } catch { }
    }
}
