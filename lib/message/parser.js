...
const { Gfs, getC2CFileUrl, getVideoUrl } = require("./file");
const pb = require("../pb");
        ...
        protocol.get(url, { headers }, (res) => {
            ...
            res.on("end", () => {
                try {
                    ...
                    buf = zlib.unzipSync(buf[3].toBuffer());
                    resolve(buf);
                } catch (e) {
    ...
    async parseExclusiveElem(type, elem) {
        ...
        case 51: //json
            ...
            if (elem[1].toBuffer()[0] > 0)
                msg.data.data = String(zlib.unzipSync(elem[1].toBuffer().slice(1)));
            else
    ...
    parseImgElem(elem) {
        ...
        if (!this.gid) { //私图
            ...
            if (elem[16])
                ...
                data.url = `https://gchat.qpic.cn/gchatpic_new/${this.uid}/${code2uin(this.gid)}-${elem[7]}-${elem[13].toHex().toUpperCase()}/0?term=2`;
        }
        return data;
    }
