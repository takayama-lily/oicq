    ...
    async parseReplyElem(elem) {
        ...
        try {
            ...
            if (this.gid) {
                ...
                let user_id = this.uid, flag = 0;
                if (user_id === this.c.uin) {
                    user_id = elem[2];
                    flag = 1;
                }
                msg.data.id = genC2CMessageId(user_id, elem[1], random, elem[3], flag);
            }
...
async function parseC2CMsg(msg, realtime = false) {
    ...
    try {
        ...
        message_id = genC2CMessageId(uin, seq, body[1][1][3], time, flag);
