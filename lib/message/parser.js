...
class Parser {
    ...
    async parseElems(elems) {
        ...
        while (true) {
            ...
            if (type === 16) { //extraInfo 额外情报
                ...
                if (elem[6] === 1 && elem[7]) {
                    const buf = await downloadMultiMsg.call(this.c, elem[7].raw, 1);
    ...
    async parseReplyElem(elem) {
        ...
        try {
            ...
            if (this.gid) {
                let m = await getGroupMsgs.call(this.c, this.gid, elem[1], elem[1]);
                m = m[0];
...
async function parseC2CMsg(msg, realtime = false) {
    ...
    if (sender.nickname === undefined) {
        const stranger = (await this.getStrangerInfo(user_id, seq % 5 == 0 && realtime)).data;
        if (stranger) {
            stranger.group_id = sender.group_id;
            Object.assign(sender, stranger);
            if (!this.sl.has(user_id) || realtime)
    ...
    if (type === 529) {
        if (head[4] !== 4)
            return;
        var parser = await parseC2CFileElem.call(this, body[2][1]);
    } else if (body[1] && body[1][2]) {
        var parser = new Parser(this, user_id, 0);
...
async function parseForwardMsg(resid) {
    const data = [];
    const blob = await downloadMultiMsg.call(this, String(resid), 2);
    /**
     * @type {import("../ref").Msg[]}
     */
