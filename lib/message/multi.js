 ...
 * @param {import("../ref").FakeMessage[]} messages 
 */
async function buildForwardMsg(messages, dm = false) {

    if (messages[Symbol.iterator] !== "function")
        messages = [messages];
    ...
    for (const msg of messages) {
        if (!msg.message || !common.checkUin(msg.user_id)) {
            this.logger.warn("skip invalid FakeMessage: " + JSON.stringify(msg));
            continue;
        }
        const elements = [];
        let brief = "";
        ...
        msg.nickname = String(msg.nickname || msg.user_id);
        nodes.push({
            1: {
                1: msg.user_id,
                2: this.uin,
        ...
        if (cnt < 4) {
            ++cnt
            preview += `<title color="#777777" size="26">${common.escapeXml(msg.nickname)}: ${common.escapeXml(brief.slice(0, 50))}</title>`;
        }
    }
    if (!nodes.length)
...
module.exports = {
    buildForwardMsg, uploadMultiMsg,
};
