...
async function qwerty(url, options = {}) {
...
async function getMiGuSongInfo(id) {
    let rsp = await qwerty(`https://c.musicapp.migu.cn/MIGUM2.0/v1.0/content/resourceinfo.do?copyrightId=${id}&resourceType=2`);
    rsp = JSON.parse(rsp).resource[0]
    let previewUrl = await qwerty(`https://music.migu.cn/v3/api/music/audioPlayer/getSongPic?songId=${rsp.songId}`, { headers: { referer: "https://music.migu.cn/v3/music/player/audio" } })
    let preview = JSON.parse(previewUrl).smallPic || ""

    let getJumpUrl = await qwerty(`https://app.c.nf.migu.cn/MIGUM2.0/v1.0/content/shareInfo.do?contentId=${rsp.copyrightId}&contentName=${rsp.songName}&resourceType=2&targetUserName=${rsp.singer}`)
    let jumpUrl = JSON.parse(getJumpUrl).url
    return {
        title: rsp.songName,
        singer: rsp.singer,
        jumpUrl,
