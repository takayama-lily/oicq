...
class Gfs {
    ...
    async upload(filepath, pid = "/", name) {
        ...
        if (!rsp[10]) {
            ...
            await highwayUploadStream.call(this.c, fs.createReadStream(filepath, { highWaterMark: 1024**2 }), {
                cmd: 71,
                md5, size, ext
            });
            await new Promise(resolve => setTimeout(resolve, 1000 + Math.ceil(size * 5 / 1e6)));
        }
