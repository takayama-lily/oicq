...
class Gfs {
    ...
    async _statFileOrBuffer(pathOrBuffer) {
        if (typeof pathOrBuffer === 'string') {
            const [[md5, sha1], { size }] = await Promise.all([
                await common.fileHash(pathOrBuffer),
        ...
        } else if (pathOrBuffer instanceof Buffer || pathOrBuffer instanceof Uint8Array) {
            return {
                size: pathOrBuffer.byteLength,
                md5: common.md5(pathOrBuffer),
                sha1: common.sha1(pathOrBuffer),
            };
        } else {
            throw new GfsError('参数既不是文件路径字符串也不是 Buffer 也不是 Uint8Array');
        }
    }

    _getStreamForFileOrBuffer(pathOrBuffer) {
        if (typeof pathOrBuffer === 'string') {
            return fs.createReadStream(pathOrBuffer, { highWaterMark: 1024 * 256 });
        } else if (pathOrBuffer instanceof Buffer || pathOrBuffer instanceof Uint8Array) {
            return Readable.from(pathOrBuffer)
        } else {
            throw new GfsError('参数既不是文件路径字符串也不是 Buffer 也不是 Uint8Array');
        }
    }

    async upload(pathOrBuffer, pid = "/", name) {
        const { md5, sha1, size } = await this._statFileOrBuffer(pathOrBuffer);
        if (name) {
            name = String(name);
            if (name.length <= 0) {
                throw new GfsError('文件名称不能为空');
            }
        } else if (pathOrBuffer instanceof Buffer || pathOrBuffer instanceof Uint8Array) {
            throw new GfsError('使用 Buffer 上传时需要手动提供文件名称');
        } else {
        ...
        if (!rsp[10]) {
            ...
            await highwayUploadStream.call(this.c, this._getStreamForFileOrBuffer(pathOrBuffer), {
                cmd: 71,
                md5, size, ext
            });
        }
