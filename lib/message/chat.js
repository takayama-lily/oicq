...
async function toForwardMsgElems(target, nodes) {
    ...
    let preview = "";
    preview += ` <title color="#000000" size="26" > 转发的聊天记录 </title>`
    ...
    return [
        {
            12: {
                1: Buffer.concat([Buffer.from([1]), zlib.deflateSync(template)]),
                2: 35,
            },
        },
        {
...
async function getHistoryMsg(message_id) { 
    if (message_id.length > 24) {
        ...
        try {
            const msg = await handleGroupMsg.call(this, o[1], o[3], false);
        ...
        try {
            const msg = await handlePrivateMsg.call(this, o[1][3], o[1], o[2], o[3]);
