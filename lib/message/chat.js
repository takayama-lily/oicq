...
async function sendPrivateMsg(user_id, rich) {
    ...
    if (this.sl.has(user_id)) {
        try {
            ...
            if (group_id && (await this.getGroupMemberInfo(group_id, user_id)).data)
                routing = {
                    3: {
...
async function toForwardMsgElems(target, nodes) {
    ...
    <msg brief="[聊天记录]" m_fileName="${common.uuid().toUpperCase()}" action="viewMultiMsg" tSum="2" flag="3" m_resid="${resid}" serviceID="35" m_fileSize="${compressed.length}"  > <item layout="1"> <title color="#000000" size="34" > 转发的聊天记录 </title>${preview}  <hr></hr> <summary color="#808080" size="26" > 查看${nodes.length}条转发消息  </summary> </item><source name="聊天记录"></source> </msg>`;
    return [
        {
            12: {
...
async function sendB77RichMsg(buf) {
    try {
        ++this.stat.sent_msg_cnt;
        await this.sendOidb("OidbSvc.0xb77_9", buf);
    } catch { }
    return { result: 0, data: { message_id: "" } };
}
...
async function getHistoryMsg(message_id) {
    if (message_id.length > 24) {
        ...
        try {
            const data = await parsePrivateMsg.call(this, msg);
