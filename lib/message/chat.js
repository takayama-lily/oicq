 ...
 * @returns {import("../ref").ProtocolResponse}
 */
async function getRoamingFaces(no_cache = false) {
    if (!this.roaming_faces.length || no_cache) {
        ...
        this.roaming_faces_bid = String(rsp[4][3].raw);
        if (rsp[4][1]) {
            const faces = Array.isArray(rsp[4][1]) ? rsp[4][1] : [rsp[4][1]];
            this.roaming_faces = faces.map(x => String(x.raw));
        }
    }
    return {
        result: 0,
        data: {
            bid: this.roaming_faces_bid,
            url: `https://p.qpic.cn/${this.roaming_faces_bid}/${this.uin}/\${face}/0`,
            faces: this.roaming_faces,
        }
    };
}
