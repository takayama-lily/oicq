 ...
 * 获取群请求
 * @this {import("./ref").Client}
 * @param {Number} type 1申请 2邀请 22申请(来自群员的邀请)
 */
async function getNewGroup(type) {
    ...
    try {
        ...
        for (let vv of o.groupmsgs) {
            if (vv.msg.subType !== 1 || vv.msg.groupMsgType !== type)
                continue;
            if (!v || toInt(vv.msgTime) > toInt(v.msgTime))
                v = vv;
        }
        if (!v) return;
        const time = toInt(v.msgTime);
        const group_id = toInt(v.msg.groupCode);
        if (Date.now() - time * 1000 > 5000) return;
        if (type === 1 || type === 22) {
            const user_id = toInt(v.reqUin);
            ...
            this.em("request.group.add", {
                ...
                nickname:   v.msg.reqUinNick,
                comment:    v.msg.msgAdditional,
                inviter_id: type === 22 ? toInt(v.msg.actionUin) : undefined,
                flag, time
            });
        } else if (type === 2) {
