...
const default_config = {
    ...
    device_filepath: process.mainModule.path //设备文件保存路径，默认为main目录下
};
...
class AndroidClient extends Client {
    ...
    constructor(uin, config = {}) {
        ...
        this.device_info = device(path.join(config.device_filepath, `device-${uin}.json`));
    ...
    login(password_md5) {
        ...
        if (password_md5) {
            ...
            // this.random_key = common.md5(common.rand().toString());
            // this.device_info.imsi_md5 = common.md5(common.rand().toString());
            // this.device_info.tgtgt_key = common.md5(common.rand().toString());
        }
        this._connect(()=>{
    ...
    async setGroupInviteRequest(flag, approve = true) {}
