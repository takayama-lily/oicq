...
const {md5, rand, buildApiRet, checkUin} = require("./packet/common");
...
class AndroidClient extends Client {
    ...
    changeOnlineStatus(status = 11) {
        ...
        this.startHeartbeat();
        this.write(outgoing.buildClientRegisterRequestPacket(status, this));
     ...
     * 返回值的形式为
     *  {
     *      retcode: 0,     //0正常 1异步 100参数错误 102调用失败
    ...
    async sendPrivateMsg(user_id, message, auto_escape = false) {
        ...
        try {
            try {
                var packet = await outgoing.buildSendFriendMessageRequestPacket(user_id, message, auto_escape, this);
            } catch (e) {
                this.logger.debug(e.stack);
                return buildApiRet(100);
            }
            ...
            if (stranger && stranger.group_id)
                return await this.sendTempMsg(stranger.group_id, user_id, message, auto_escape, this);
    ...
    async sendTempMsg(group_id, user_id, message, auto_escape = false) {
        ...
        try {
            try {
                var packet = await outgoing.buildSendTempMessageRequestPacket(group_id, user_id, message, auto_escape, this);
            } catch (e) {
                this.logger.debug(e.stack);
                return buildApiRet(100);
            }
